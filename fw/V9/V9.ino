...
const String CMD_RESET_WIFI = "CMD_RESET_WIFI";
const String CMD_REBOOT = "CMD_REBOOT";
const String CMD_RGB_BRI = "CMD_RGB_BRI_";
const String CMD_OLED_BRI = "CMD_OLED_BRI_";
...
int rgb_brightness = 24;
int oled_brightness = 192;
...
void setupOTA() {
  const char* identifier = ("AG-" + String(getNormalizedMac())).c_str();

  ArduinoOTA.onStart([]() {
    Serial.println("Start ArduinoOTA");
  });
  ArduinoOTA.onEnd([]() {
    Serial.println("\nEnd");
  });
  ...
  //This is less of a security measure and more an accidential flash prevention
...
void sendToServer() {
  if (currentMillis - previoussendToServer >= sendToServerInterval) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      if (CMD_RESET_WIFI.equals(response)) {
        Serial.println("Received WIFI_RESET command");

        wifiManager.resetSettings();
        delay(5000);
        ESP.restart();
      } else if (CMD_REBOOT.equals(response)) {
        Serial.println("Received REBOOT command");

        ESP.restart();
      } else if (response.startsWith(CMD_RGB_BRI)) {
        Serial.println("Received RGB_BRI command");
        long newRgbBri = response.substring(CMD_RGB_BRI.length()).toInt();

        if(newRgbBri < 0) {
          newRgbBri = 0;
        } else if(newRgbBri > 255) {
          newRgbBri = 255;
        }

        rgb_brightness = int(newRgbBri);
      ...
      } else if (response.startsWith(CMD_OLED_BRI)) {
        Serial.println("Received OLED_BRI command");
        long newOledBri = response.substring(CMD_OLED_BRI.length()).toInt();

        if(newOledBri < 0) {
          newOledBri = 0;
        } else if(newOledBri > 255) {
          newOledBri = 255;
        }

        oled_brightness = int(newOledBri);
